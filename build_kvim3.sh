...
DEVICE_PATH="${LINEAGE_ROOT}/device/khadas/kvim3"
VENDOR_PATH="${LINEAGE_ROOT}/vendor/khadas/kvim3"
...
if [[ -d "${VENDOR_PATH}" ]]; then
    cd "$VENDOR_PATH" && git add -A && git commit -m "kvim3: Update bootloader image prebuilt"
fi
