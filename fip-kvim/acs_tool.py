#!/usr/bin/env python3
# Python 3 port of Amlogic's fip/acs_tool.pyc (acs_tool.py v1, 2015-09-17,
# Python 2.7 bytecode): merge the board acs.bin (DDR/DDR-timing/PLL settings
# built from board/*/firmware/timing.c) into bl2.bin.
#
# usage: acs_tool.py <bl2.bin> <bl2_out.bin> <acs.bin> <debug 0|1>
import collections
import copy
import shutil
import sys
...
class acs_tool(object):
    ...
    def init_acs(self, acs_struct, file_name, bl2):
        with open(file_name, 'rb') as f:
            ...
            acs_entry_point, = unpack('<H', f.read(2))
            ...
            for key in list(acs_struct.keys()):
                ...
                if isinstance(acs_struct[key], str):
                    ...
                    acs_struct[key] = f.read(len(acs_struct[key])).decode('latin-1')
                elif isinstance(acs_struct[key], int):
                    seek_position += acs_struct[key]
                    if acs_struct[key] == 1:
                        acs_struct[key], = unpack('<B', f.read(1))
                    else:
                        acs_struct[key], = unpack('<H', f.read(2))
    ...
    def copy_data(self):
        with open(self.file_des_tmp, 'r+b') as file_des, open(self.file_src, 'rb') as file_src:
